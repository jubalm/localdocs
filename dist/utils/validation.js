...
exports.validateAndCleanTags = validateAndCleanTags;
exports.isValidUrl = isValidUrl;
exports.isValidHashId = isValidHashId;
exports.isValidExportFormat = isValidExportFormat;
exports.validateDocumentMetadata = validateDocumentMetadata;
...
function validateDocumentMetadata(metadata) {
    if (!metadata || typeof metadata !== 'object') {
        throw new index_js_1.ValidationError('Document metadata must be an object');
    }
    if (!metadata.url || typeof metadata.url !== 'string') {
        throw new index_js_1.ValidationError('Document metadata must have a valid URL');
    }
    if (!isValidUrl(metadata.url)) {
        throw new index_js_1.ValidationError(`Invalid URL format: ${metadata.url}`);
    }
    if (metadata.name !== undefined && metadata.name !== null && typeof metadata.name !== 'string') {
        throw new index_js_1.ValidationError('Document name must be a string');
    }
    if (metadata.description !== undefined && metadata.description !== null && typeof metadata.description !== 'string') {
        throw new index_js_1.ValidationError('Document description must be a string');
    }
    if (metadata.tags !== undefined && metadata.tags !== null) {
        if (!Array.isArray(metadata.tags)) {
            throw new index_js_1.ValidationError('Document tags must be an array');
        }
        for (const tag of metadata.tags) {
            if (typeof tag !== 'string') {
                throw new index_js_1.ValidationError('All tags must be strings');
            }
        }
    }
}
