...
export class InteractiveManager {
   ...
   */
  private collectAvailableTags(): void {
   ...
   */
  private getFilteredDocs(): Record<string, any> {
    const config = this.manager.getConfig();
  ...
  private renderInterface(): void {
    ...
    this.state.terminalSize = getTerminalSize();
   ...
   */
  private renderTreeLayout(width: number): void {
    for (let i = 0; i < this.state.docs.length; i++) {
      ...
      if (description.length <= descWidth) {
        ...
        const wrappedLines = wrapText(description, descWidth);
   ...
   */
  private renderColumnLayout(width: number): void {
    ...
    if (remainingWidth > 20) { // Ensure minimum space
      ...
      nameWidth = Math.min(maxNameLength + 2, Math.floor(remainingWidth / 2), MAX_NAME_COLUMN_WIDTH);
      nameWidth = Math.max(nameWidth, MIN_COLUMN_WIDTH + 2);
      descWidth = Math.max(MIN_COLUMN_WIDTH + 2, remainingWidth - nameWidth);
    } else {
      // Very narrow terminal - minimal widths
      nameWidth = MIN_COLUMN_WIDTH;
      descWidth = Math.max(MIN_COLUMN_WIDTH, remainingWidth - nameWidth);
    }
    ...
    for (let i = 0; i < this.state.docs.length; i++) {
      ...
      const displayName = truncateText(name, nameWidth);
      const displayDesc = truncateText(description, descWidth);
   ...
   */
  private renderControls(width: number): void {
    ...
    const minSpacing = MIN_SPACING; // Comfortable spacing between controls
    const horizontalPadding = HORIZONTAL_PADDING; // 2 spaces on each side for breathing room
    ...
    if (requiredWidth <= width) {
      ...
      console.log(buildCenteredLine(line1, width));
      console.log(buildCenteredLine(line2, width));
    }
  }
   ...
   */
  private async handleKey(key: KeyPress): Promise<boolean> {
   ...
   */
  private async handleSetMetadata(): Promise<void> {
    if (this.state.docs.length === 0 || this.state.currentIndex >= this.state.docs.length) {
      return;
    }
   ...
   */
  private async handleTagFilterMode(): Promise<void> {
   ...
   */
  private async handleQuitConfirmation(): Promise<boolean> {
    clearScreen();
